...
naming that filesystem rather than falling through to something wrong. An unqualified
live scan (no `--technique`) never attempts a live `slack-space` detect, the same way
it skips any technique a filesystem doesn't support; `--technique slack-space`
explicitly does, opening the volume's raw device read-only (root only if that device
node isn't already readable by the current user). See [APFS live
